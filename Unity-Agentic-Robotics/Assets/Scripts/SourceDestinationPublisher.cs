...
public class SourceDestinationPublisher : MonoBehaviour
{
    ...
    [SerializeField] 
    private ArticulationBody[] robotJoints = new ArticulationBody[k_NumRobotJoints];
 ...
 IEnumerator SetJointValues(JointStates message)
{
    ...
    Debug.Log("Message Names: " + string.Join(", ", message.name));

    for (int i = 0; i < message.name.Length; i++)
    {
        // Check if the index is within the bounds of the robotJoints array
        if (i >= robotJoints.Length || robotJoints[i] == null)
        {
            Debug.LogError($"Index {i} is out of bounds or not assigned in the robotJoints array.");
        ...
        Debug.Log($"Joint {i}: Angle set to {jointAngle} degrees");
    }
    ...
    if (robotJoints != null)
    {
        foreach (var joint in robotJoints)
        {
            if (joint != null)
            {
                ...
                Gizmos.DrawSphere(joint.transform.position, 0.02f);
            }
        }
    }
}
